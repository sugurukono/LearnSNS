<?php
    ...
    $sql = 'SELECT * FROM `self_users` WHERE `id` = ?';
    ...
    if (!empty($_POST)) {
        ...
        if ($feed == '') {
            ...
            $sql = 'INSERT INTO `self_feeds` SET `user_id` = ?, `feed` = ?, `created` = NOW()';
    ...
    //一覧データの取得
    $sql = 'SELECT `f`.*, `u`.`name`,`u`.`img_name` AS `profile_image` FROM `self_feeds` AS `f` INNER JOIN `self_users` AS `u` ON `f`.`user_id` = `u`.`id` ORDER BY `created` DESC'; 
    //一覧を取得するSELECT文を記述
        ...
        <ul class="nav navbar-nav navbar-right">
          <li class="dropdown">
            ...
            <ul class="dropdown-menu">
              ...
              <li><a href="self_signout.php">サインアウト</a></li>
            </ul>
          </li>
        </ul>
      </div>
